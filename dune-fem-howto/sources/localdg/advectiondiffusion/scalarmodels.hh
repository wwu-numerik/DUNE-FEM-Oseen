...
class BurgersModel {
 ...
 public:
  ...
  {
    problem_.velocity(time,en.geometry().global(x),f[0]);
    f[0] *= u*u*0.5;
  }
  ...
  {
    problem_.velocity(time,en.geometry().global(x),velocity);
  }
...
class AdvectionDiffusionModel {
 ...
 public:
  AdvectionDiffusionModel(GridType& grid,
        const ProblemType& problem) :
    problem_(problem),
    velocity_( problem.constantVelocity() ), 
    epsilon(problem.epsilon), 
    tstep_eps((problem.diff_tstep)?problem.epsilon:0) 
  {
  }
  ...
  inline  void velocity(
             ...
             DomainType& v) const 
  {
    problem_.velocity(time,en.geometry().global(x),v);
  }
  ...
  inline void maxSpeed(const typename Traits::DomainType& normal,
           double time,  
           const typename Traits::DomainType& xgl,
           const RangeType& u,
           double& advspeed,double& totalspeed) const 
  {
    problem_.velocity(time, xgl, velocity_);
...
class UpwindFlux<AdvectionDiffusionModel<GridPartType,ProblemType> > {
 ...
 protected: 
  ...
  // velocity evaluation for constant velocities  
  template <class Model>
