...
// Deprecated constructor with two arguments.  The code is identical to the
// one with only the heapSize as an argument.
template <int dim>
inline Dune::UGGrid < dim >::UGGrid(unsigned int heapSize, unsigned int envHeapSize)
    : multigrid_(NULL), 
      leafIndexSet_(*this),
      globalIdSet_(*this),
      localIdSet_(*this),
