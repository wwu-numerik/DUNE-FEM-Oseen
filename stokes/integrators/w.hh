			...
			void applyVolume( const InfoContainerVolumeType& info )
			{
				...
				for ( int i = 0; i < info.info.numSigmaBaseFunctionsElement; ++i ) {
					for ( int j = 0; j < info.info.numVelocityBaseFunctionsElement; ++j ) {
						...
						for ( size_t quad = 0; quad < info.info.volumeQuadratureElement.nop(); ++quad ) {
							// get x
							const ElementCoordinateType x = info.info.volumeQuadratureElement.point( quad );
							// get the integration factor
							const double elementVolume = info.geometry.integrationElement( x );
							// get the quadrature weight
							const double integrationWeight = info.info.volumeQuadratureElement.weight( quad );
							// compute v_j^t \cdot ( \nabla \cdot \tau_i^t )
							VelocityRangeType v_j( 0.0 );
							info.info.velocityBaseFunctionSetElement.evaluate( j, x, v_j );
